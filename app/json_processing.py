...
def parse_json_and_set_form(db: SQLAlchemy, new_form: Formulario, current_form):
    ...
    for key, value in current_form.items():
        if key != 'bienRaiz' and key in main_keys:
            ...
            try:
                ...
                setattr(new_form, 'fecha_inscripcion', parsed_date)
            except ValueError:
...
def parse_json_and_save_form_in_db(db: SQLAlchemy, new_form, single_form):
    parse_json_and_set_form(db, new_form, single_form)
    db.session.add(new_form)
    db.session.commit()


def parse_json_and_get_enajenantes(db: SQLAlchemy, current_form):
    ...
    for single_enajenante in current_form['enajenantes']:
        ...
        if 'RUNRUT' in single_enajenante.keys():
            ...
            person = db.session.query(Persona).filter_by(
...
def parse_json_and_save_enajenantes_in_db(db: SQLAlchemy, single_form, new_form_id):
    enajenantes: List[Enajenante] = parse_json_and_get_enajenantes(db, single_form)
...
def parse_json_and_save_adquirientes_in_db(db: SQLAlchemy, single_form, new_form_id):
    adquirientes: List[Adquirente] = parse_json_and_get_adquirentes(db, single_form)
...
def parse_json_and_get_adquirentes(db: SQLAlchemy, current_form):
